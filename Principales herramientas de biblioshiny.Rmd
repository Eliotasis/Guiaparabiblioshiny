...
[^1]: La búsqueda avanzada en Scopus implica el empleo de comandos de búsqueda basados en códigos, y está destinada a realizar búsquedas muy complejas, que exijan tal combinación de elementos, que las anteriores opciones de búsqueda no sean efectivas. Dado que Scopus no permite compartir búsquedas a través de enlaces, comparto aquí el código de la búsqueda avanzada. Con pegar este texto en la barra de búsqueda deberías obtener los mismos resultados. 
...
Biblioshiny nos permite eliminar algunos datos de nuestra muestra dependiendo de nuestros intereses a través de la pestaña `Filter`. Entre las opciones que nos ofrece nos permite filtrar por:
...
En `Dataset>Main Information` obtenemos información cuantitativa sobre el set de artículos que hemos cargado a través de una tabla([ver anexo](#glosario)).
...
Esta representación nos permite observar de un vistazo cuales son las referencias mas citadas, con qué autores conecta y qué palabras clave tiene relación. Los puntos débiles de esta combinación es que la longitud de los títulos de las referencias solapan con con el campo central, lo que resulta confuso. Recomiendo interactuar con el diagrama colocando el ratón sobre las distintas unidades representadas, ya que con esta acción se acentúan las interacciones de dichos elementos y permite sacar mejores conclusiones. 


## Fuentes. 

Una fuente es una revista, libro, capítulo de libro, conferencia o cualquiera que sea el origen de dicha información. El análisis de las fuentes puede ser interesante para observar cuales son las que más impacto tienen en nuestro campo de estudio y puede guiar búsquedas futuras. Identificar cuales son las fuentes más relevantes puede hacerse siguiendo múltiples estrategias, biblioshiny nos ofrece 5 herramientas distintas para medir distintas características de lo que se puede considerar una fuente importante. 

### Fuentes más relevantes. 

En la pestaña `Sources>Most relevant Sources` nos muestra la cantidad de documentos que presentan las fuentes de nuestra muestra bibliográfica. 
...
En la pestaña `Sources>Most Local Cited Sources` nos muestra un diagrama con las fuentes localmente más citadas. Las fuentes localmente más citadas son aquellas que aparecen en la bibliografía de los documentos de nuestra muestra bibliográfica ([ver anexo](#terminos)). 
...
Podemos realizar un cálculo de las revistas que forman parte del "núcleo" a través de la ley de bradford ([ver anexo](#bradford)), que se encuentra en `Sources>Bradford´s law`.  

 En biblioshiny puedes aplicar la ley de bradford en la pestaña `Sources>Bradford's Law`.
...
Biblioshiny nos permite medir el impacto de las fuentes a través de 4 parámetros([ver anexo](#indices)):
...
El índice $h$ de las fuentes, así como la ley de bradford, nos vuelve a indicar que las fuentes a las que debemos prestar mas atención son ECOLOGY, OIKOS, ECOLOGY LETTERS y JOURNAL OF ANIMAL ECOLOGY. La ley de bradford nos informaba de que estas revistas son las que forman parte del núcleo de la investigación, mientras que el índice $h$ nos informa de que, además, son las que tienen más impacto. 
...
Si elegimos `M-Index`, obtendremos ahora el diagrama para el índice $m$ de impacto. Existe una fuente con un nombre muy largo que nos comprime el diagrama hacia la derecha, como este es el último reducimos el número de fuentes representadas a 11 para obtener un diagrama más apaisado. 
...
La fuente más importante de todas en el ámbito de las redes mutualistas según nuestra muestra es OIKOS por las siguientes razones: 

- Mayor número de artículos. 
- El análisis de bradford lo coloca en el centro del núcleo de las fuentes mas relevantes. 
- Mayor índice $g$ y $m$ lo que nos habla de que presenta mas artículos muy citados y mas impacto en relación al tiempo que lleva la revista en activo, respectivamente. 

La segunda fuente mas importante a la que deberíamos presentar también atención es ECOLOGY:

- Presenta menos número de artículos pero localmente es mas citado que OIKOS. 
- Presenta mayor índice $h$ que OIKOS, lo que nos informa de que sus artículos presentan en promedio más citas que OIKOS, aunque este gana a ECOLOGY en poseer algunos artículos muy citados. 

La tercera y cuarta revista más importantes son ECOLOGY LETTERS y JOURNAL OF ANIMAL ECOLOGY. Esto debido a que ECOLOGY LETTERS gana en todos los aspectos estudiados (número de artículos, proximidad al núcleo de bradford, índices $h$, $g$ y $m$). Mencionar que las revistas NATURE y SCIENCE aparecen como la tercera y cuarta revista localmente mas citadas, pero no aparecen en ninguno de los otros análisis como revistas importantes. Debido al alto impacto estas revistas es posible que presenten artículos no directamente relacionados con el tema de las redes mutualistas pero que haya influido en su evolución. 

## Autores. 

La búsqueda de los autores más relevantes puede seguirse bajo varios criterios que observaremos a continuación. Conocer a los autores más importantes de una disciplina es uno de los aspectos más importantes a la hora de conocer un determinado campo de estudio. 

### Autores más relevantes. 

En la pestaña `Authors>Most Relevant Authors` podemos realizar 3 análisis distintos relacionados con la relevancia de los autores. Estos análisis se pueden alternar a partir del menú desplegable de la izquierda `Frequency measure`, donde podemos elegir entre representar número de documentos, porcentaje del total de documentos o frecuencia fraccionada([ver anexo](#frecfrac)). 

![Diagrama de los autores mas relevantes por número de documentos](MostRelevantAuthorsND.png)

Bascompte, Vzquez y Jordano son los autores más productivos, por lo que puede ser interesante prestarles atención. Con la frecuencia fraccionada estos autores principales no cambian su posición, por lo que podemos decir que además de presentar muchos artículos, estos presentan pocos coautores, de lo que se puede deducir que probablemente realizaron una alta contribución a sus respectivos trabajos. 
...
En la pestaña `Authors>Lotka's Law` podemos medir la productividad de los autores de nuestra muestra y observar si nuestro modelo se ajusta a Ley de Lotka ([ver anexo](#lotka)). 
...
En la pestaña `Table` nos muestra una tabla con los países ordenados por frecuencia productiva. 
...
Estados Unidos y Brasil son países más productivos, pero España resulta ser la más citada, probablemente resultado de que los autores mas citados provienen de este país.

### Conclusiones sobre los autores. 

El autor más importante según nuestra muestra bibliográfica es Bascompte ya que aparece primero en todos nuestros análisis, lo que lo convierte en:

- El autor más productivo. 
- El autor localmente más citado. 
- El autor con mas producción a lo largo del tiempo. 
- El autor con el mayor impacto según los índices $h$, $g$ y $m$. 

El segundo autor más importantes es Vzquez debido a que es:

- El segundo autor más productivo. 
- El segundo autor con más impacto según los índices $h$ y $g$. 
- Además aparece como el cuarto autor con más producción a lo largo del tiempo. 

El tercer autor más importante es Jordano debido a que es:

- El tercer autor mas productivo. 
- El tercer autor localmente más citado. 
- El tercer autor con más impacto según los índices $h$ y $g$. 
- Además aparece como el segundo autor con más producción a lo largo del tiempo. 

El cuarto autor más importante es Blthgen debido a que es: 
- El cuarto autor más productivo. 
- El cuarto autor con más impacto según los índices $h$ y $g$. 
- Además aparece como el segundo autor localmente más citado. 

Además de estos autores, caben destacar:

- Olessen, que aparece como el cuarto autor localmente más citado. 
- Chacoff, que aparece como el tercer autor con más producción a lo largo del tiempo. 
- Burkela, Guimares y Dttilo, que aparecen segundo, tercero y cuarto en autores con más impacto según el índice $m$ respectivamente. Esto quiere decir que pueden tratarse de autores importantes pero con una trayectoria infrarrepresentada por su poca longitud en el tiempo. 

En cuanto a los países más importantes en este campo tenemos USA, Brasil, España, Argentina y Alemania. 

- La mayoría de los autores son estadounidenses, además Estados Unidos es el más productivo y el segundo más citado. 

- Brasil es el segundo país con más autores y segundo más productivo. Se trata del quinto país mas citado. 

- España es el tercer país con mas autores y el tercero más productivo. Curiosamente, se trata del país más citado, probablemente debido a los trabajos de Bascompte. 

- Argentina es el cuarto país con más autores, el cuarto más productivo y el quinto más citado. 

- Alemania es el quinto país que más autores ofrece, el quinto más productivo y el cuarto más citado. 

Estados unidos presenta relativamente menos nivel de colaboración con otros países que los demás. Brasil y España presenta una colaboración de casi el 50% de sus documentos con otros países, mientras que Argentina y Alemania presenta colaboraciones en más de la mitad de sus artículos. 
...
En la pestaña `Documentos>Reference Spectroscopy` podemos realizar una espectroscopía de los años de publicación de las referencias citadas ([ver anexo](#rpys)). Existe una referencia citada de 1713 que desvirtúa el gráfico. Para conseguir que el gráfico esté menos comprimido hacia la izquierda localizamos el siguiente año más citado y hacemos el análisis a partir de él. En nuestro caso elegimos 1859, que tiene un total de 5 referencias citadas, y lo especificamos en el menú de la izquierda en `TimeSlice>Starting Year`. 

![Espectroscopía de las referncias](ReferencePublicationYearSpectroscopy.png)

A través de esta espectroscopía es interesante observar los picos, ya que representan años donde se publicaron referencias que fueron muy citadas. Nuestro conjunto de artículos proviene de los años 2002:2020, por lo que para localizar las raíces de un tema puede ser interesante observar los artículos mas citados de los años anteriores a 2002, porque serán las influencias de nuestro campo de estudio. Entre 1955:2000 podemos localizar 9 picos cuyas referencias sería interesante revisar para localizar las fuentes de nuestro tema.  
...
- Medida de la ocurrencia de palabras: Podemos elegir que las palabras sean tan grandes como la frecuencia a la que aparecen o que este tamaño pase primero por una función de raíz cuadrada, de logaritmo base 2 o base 10. 
...
Podemos observar que los dos temas mas productivos son el mutualismo y la polinización. Le sigue ecosistemas, animales, simbiosis y dispersion de semillas. Por ultimo tenemos interacción planta polinizado y anidamiento. Todos estos son los conceptos de moda sobre los que puede ser interesante investigar. 

### Términos de moda. 

En la pestaña `Documents>Trend Topics` podemos generar un diagrama con los términos de moda, en qué momento aparecen y cuando desaparece, con una serie de burbujas asociadas que describen el momento en el que mas aparecen y con qué frecuencia, siendo más grandes cuanto más veces aparecieran en ese marco temporal. 
...
### Conclusiones sobre los documentos. 

El análisis de citas nos ha dado información relevante sobre cuales son los artículos más citados, tanto globalmente como localmente. Algunos de los artículos interesantes podrían ser: 

- Bascompte J 2007. El artículo globalmente y localmente más citado. 
- Ings TC 2009. El segundo artículo globalmente más citado. 
- Jordano P 2009. El tercer artículo globalmente más citado y el segundo localmente más citado. 
- Olessen JM 2002. El cuarto artículo globalmente más citado.
- Bascompte J 2009. Tercer artículo localmente más citado. 
- Vzquez 2009. Cuarto artículo localmente más citado. 

El análisis de los artículos más citados parece arrojar artículos que se encuentran principalmente en el año 2009 y otros de años anteriores. Para hacernos una mejor idea de la evolución completa del tema de las redes mutualistas hasta la actualidad sería aconsejable realizar un análisis pormenorizado de cada uno de los años, o de marcos temporales de varios años entre 2002 y 2020. 

La espectroscopía de las referencias nos ha dado información sobre los años con mas citas en las referencias de nuestros artículos. Puede ser interesante visitar cada uno de esos años para observar cuales son los artículos más citados y consultarlos. Esto nos puede permitir hacernos una idea de cuales fueron las raíces teóricas de nuestro tema. Esto, complementado con una lectura de los artículos mas relevantes entre 2002 y 2020, junto con los análisis de las palabras más frecuentes, dinámica de las palabras y términos de moda, nos ofrece una oportunidad para realizar un análisis de la evolución teórica de nuestro campo. Estos últimos análisis relacionados con las palabras usadas en nuestros artículos parecen darnos pistas sobre los temas mas relevantes, entre ellos las interacciones, el  en modelos reticulares, el mutualismo y las interacciones entre plantas y animales. 

## Estructura conceptual. 
### Análisis de la red. 

El análisis de la red podemos realizarlo principalmente fijándonos en dos tipos de análisis, el mapa temático y la evolución temática. Para observar el mapa temático podemos acceder a `Conceptual Srtucture>Thematic Map`. En la primera pestaña `Map` podemos observar un mapa de densidad vs centralidad ([ver anexo](#callon)) con los principales agrupamientos de las palabras de nuestro conjunto bibliográfico. Se ha cambiado el análisis a 2000 palabras para obtener una representación con más grupos. Encontramos dos grupos básicos de temas:
...
En la pestaña `Conceptual Structure>Factorial Analysis` podemos realizar un análisis factorial ([ver anexo](#factorial)) de distintos elementos como:
...
En la pestaña `Intelectual Structure>Co-citation Network` podemos generar redes de co-citación de artículos ([ver anexo](#cocita)), autores y papers. Pondremos como ejemplo una red de co-citación de autores, que puede darnos información sobre qué grupos de autores interactuan mas entre ellos. 

![Red de co-citación de los autores](RedDeCocitaciónAutores.png)

Podemos observar que existen 4 autores que interactúan mucho entre sí: Bascompte, Jordano, Vazquez y Olesen. Alrededor de ellos encontramos un conjunto de autores que presenta muchas conexiones con estos autores. Encontramos otrso dos grupos, uno liderado por Memmot y otro por Guimares y Thompson. Sin embargo, estos otros dos grupos parecen presentar menos conexiones entre ellos que con el grupo de Bascompte, además sus autores son menos productivos. 

### Historiografía. 

La historiografía([ver anexo](#historiografia)) une los artículos como en la red de co-citación pero además los ordena en función de su fecha de publicación. Esto nos permite seguir la evolución de los temas. En la pestaá `Intellectual Structure>Historiograph` podemos generar una historiografía de nuestra muestra bibliográfica. Recomiendo bajar el número de nodos a 10 para hacer un diagrama más legible. Tambien es interesante cambiar la etiqueta de los nodos por el titulo del documento, ya que una rápida lectura de los titulos puede ayudarnos a inferir superficialmente la evolución de los temas. 

![Historiografía](Historiografia.png)

El primer artículo del que surgen muchos otros habla sobre la distribución del grado en redes mutualistas planta polinizador. Esto parede derivar en multitud de temas, entre los que tenemos:

- A)Efecto de la fragmentación del habitat en las redes hormiga-planta del amazonas. 
...
El artículo A) deriva a su vez en otro artículo que habla sobre la estructura de las redes y su papel en la estabilidad de las comunidades (A1). A1 deriva a su vez en un grupo de artículos bien interconectado:

- A1a)El role de las aves frugivoras en las redes de dispersión tiene un papel en la divergencia evolutiva?
- A1b)La presencia de peces limiadores aumenta la presencia del pez damisela en arrecifes de coral. Curiosamente este artículo no parece relacionarse con las redes mutualistas, pero dado que parece derivar de este tema puede ser interesante echarle un vistazo para ver si aporte al campo de estudio o se nos ha colado en el análisis. 
- B) El artículo B tambien conecta con el A1. 
...
En la pestaña `Social Structure>Collaboration Network` podemos generar una red de colaboración ([ver anexo](#colaboracion)).  A diferencia de las redes de co-citación, las redes colaboración unen nodos (generalmente autores) cuando presentan artículos donde ambos figuran como co-autores. 

![Red de colaboración](RedDeColaboracion.png)

La red ha sido alterada para mostrar los conjuntos mas cercanos entre sí. Parece ser que las redes de colaboración presentan mayor tendencia a fragmentarse, probablemente porque es más sencillo citar a otro autor que colaborar con él. Las colaboraciones acaban siendo más restringidas y formando redes de autores separadas que tiene a colbaorar entre sí. Los dos grupos mas grandes son el de Bascompte y Vazquez. Bascompte tienen menos nodos pero mas grandes, son pocos autores pero más contributivos, mientras que ocurre al contrario con el de Vazquez. El grupo de Bascompte sí presenta interacciones con otros grupos como el de Thompson o el de Gonzalez Castro. 

### Mapa de colaboración. 

Aunque la red de colaboración permite generar una red de colaboraciones entre los paises de los autores es preferible acceder a `Social Structure>Collaboration Map`. Esta herramienta de Biblioshiny permite generar un Mapa Mundi  donde los paises aparecen en tonos de azul en función de la cantidad de artículos producidos y aparecen lineas que unen los países que han contribuido, mostrando en su grosor la cantidad de colaboraciones. He aumentado el número mínimo de enlaces a 6 para hacer el mapa más legible.  

![Mapa de colaboraciones](CountryCollaborationMap-2022-01-05.png)

Las 10  colaboraciones más frecuentes se pueden observar en la siguiente tabla. 
...
## <a name="glosario">Términos utilizados en la información principal. </a>
...
## <a name="cagr">Ratio de crecimiento anual compuesto. </a>
...
## <a name="bradford"> Ley de bradford.  </a>
...
## <a name="indices"> Índices bibliométricos.  </a>
...
## <a name="frecfrac">Frecuencia fraccionalizada. </a> 
...
## <a name="terminos">Terminología en relación a los documentos y las referencias. </a>
...
## <a name="rpys">Espectroscopía del año de publicación de las referencias.</a>
...
### <a name="factorial">Enfoque Factorial.</a>
...
#### <a name="callon">Análisis de redes. Densidad vs Centralidad.</a>
